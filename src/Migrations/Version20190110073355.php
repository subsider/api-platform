 ...
 */
final class Version20190110073355 extends AbstractMigration
{
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, name, password, email, username, roles, password_change_date, enabled, confirmation_token FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name VARCHAR(255) NOT NULL COLLATE BINARY, password VARCHAR(255) NOT NULL COLLATE BINARY, email VARCHAR(255) NOT NULL COLLATE BINARY, username VARCHAR(255) NOT NULL COLLATE BINARY, roles CLOB NOT NULL COLLATE BINARY --(DC2Type:simple_array)
        ...
        $this->addSql('INSERT INTO user (id, name, password, email, username, roles, password_change_date, enabled, confirmation_token) SELECT id, name, password, email, username, roles, password_change_date, enabled, confirmation_token FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE INDEX IDX_9474526CA77FBEAF ON comment (blog_post_id)');
        $this->addSql('CREATE INDEX IDX_9474526CF675F31B ON comment (author_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, name, username, password, email, roles, password_change_date, enabled, confirmation_token FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name VARCHAR(255) NOT NULL, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, roles CLOB NOT NULL --(DC2Type:simple_array)
        ...
        $this->addSql('INSERT INTO user (id, name, username, password, email, roles, password_change_date, enabled, confirmation_token) SELECT id, name, username, password, email, roles, password_change_date, enabled, confirmation_token FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
    }
}
